...
function createChatSource(chatSourceConfig: ChatSourceConfig, mediaGenerators: MediaGenerator[]): ChatSource {
    if (chatSourceConfig.type === "discord") {
        return new DiscordChatSource(
            ...
            chatSourceConfig.prompt ? chatSourceConfig.prompt : null,
...
export function createChatSources(chatSourceConfigs: Array<ChatSourceConfig>, mediaGenerators: MediaGenerator[]) {
    const chatSources: Map<string, ChatSource> = new Map();
    for (const chatSourceConfig of chatSourceConfigs) {
        chatSources.set(chatSourceConfig.name, createChatSource(chatSourceConfig, mediaGenerators));
    }
    return chatSources;
}
